namespace Microsoft.ApplicationInsights.TraceListener.Tests
{
    ...
    public sealed class ApplicationInsightsTraceListenerTests : IDisposable
    {
        ...
        [TestMethod]
        [TestCategory("TraceListener")]
        public void TraceListenerWriteUsedApplicationInsightsConfigInstrumentationKeyWhenUnspecifiedInstrumentationKey2()
        {
            ...
            TraceOptions options)
        {
            ...
            using (var listener = new ApplicationInsightsTraceListener(instrumentationKey))
            {
                ...
                var traceEventCacheTypeInfo = traceEventCache.GetType().GetTypeInfo();
                traceEventCacheTypeInfo.GetDeclaredField("_timeStamp").SetValue(traceEventCache, DateTime.Now.Ticks);
