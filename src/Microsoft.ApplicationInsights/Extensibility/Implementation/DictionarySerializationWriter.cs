namespace Microsoft.ApplicationInsights.Extensibility.Implementation
{
    ...
    {   
        ...
        private long currentIndex = 1; // Index is appended to "Key" to provide a property name for objects with no name
        ...
        {            
            return string.IsNullOrEmpty(currentPrefix) ? fieldName : string.Concat(this.currentPrefix, ".", fieldName);
        }

        private string GenerateSequencialPrefix()
        {
            return GetKey(DefaultKey + this.currentIndex++);
        }

        private string GenerateSequencialPrefixForObject()
        {
            return GetKey(DefaultObjectKey + this.currentIndex++);
        }

        public void WriteProperty(string name, string value)
        {
            if (string.IsNullOrEmpty(name)) { throw new ArgumentException("This argument requires a non-empty value", nameof(name)); };
        ...
        public void WriteProperty(string name, ISerializableWithWriter value)
        {
            ...
            if (value != null)
            {
                this.lastPrefix.Push(currentPrefix);
                this.currentPrefix = GetKey(name);
                value.Serialize(this);
                this.currentPrefix = this.lastPrefix.Pop();
            }
            else
            {
                this.WriteProperty(GetKey(name), (string)null);
            }
        }
        ...
        public void WriteProperty(string name, IList<ISerializableWithWriter> items)
        {
            ...
            if (items != null)
            {
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    {
                        this.lastPrefix.Push(this.currentPrefix);
                        this.currentPrefix = GetKey(name + i);
                        items[i].Serialize(this);
                        this.currentPrefix = this.lastPrefix.Pop();
                    }
                    else
                    {
                        this.AccumulatedDictionary[GetKey(name + i)] = null;
                    }
                }
            }
            else
        ...
        public void WriteProperty(string name, IDictionary<string, string> items)
        {
            ...
            if (items != null)
            {
                ...
                {
                    string key = GetKey(string.Concat(name, ".", pair.Key));
                    if (!this.AccumulatedDictionary.ContainsKey(key))
                    {
        ...
        public void WriteStartObject(string name)
        {
            ...
            this.lastPrefix.Push(this.currentPrefix);
            this.currentPrefix = GetKey(name);
            this.lastIndex.Push(currentIndex);
            this.currentIndex = 1;
        }
        ...
        {
            this.lastPrefix.Push(this.currentPrefix);
            this.currentPrefix = GenerateSequencialPrefixForObject();
            this.lastIndex.Push(currentIndex);
            this.currentIndex = 1;
        }
