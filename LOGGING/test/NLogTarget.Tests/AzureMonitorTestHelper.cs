...
namespace Microsoft.ApplicationInsights.NLogTarget.Tests
{
    ...
    internal sealed class TelemetryCollector
    {
        ...
        private readonly List<AzureMonitorTelemetryEnvelope> itemsWithoutOperation = new();
        private int totalEnvelopesSeen;
            ...
            get
            {
                lock (this.gate)
                {
                    return this.GetCurrentOperationItemsUnsafe()
                        .Concat(this.itemsWithoutOperation)
                        .ToList();
                }
            }
        }
        ...
        public void Clear()
        {
            lock (this.gate)
            {
                ...
                this.itemsWithoutOperation.Clear();
                this.currentOperationId = null;
                this.totalEnvelopesSeen = 0;
            }
        }

        public int TotalEnvelopesSeen
        {
            get
            {
                lock (this.gate)
                {
                    return this.totalEnvelopesSeen;
                }
            }
        }
        ...
        internal void AddRange(IEnumerable<AzureMonitorTelemetryEnvelope> telemetryItems)
        {
            lock (this.gate)
            {
                foreach (var item in telemetryItems)
                {
                    ...
                    {
                        this.totalEnvelopesSeen++;
                        continue;
                    }

                    if (string.IsNullOrEmpty(item.OperationId))
                    {
                        this.itemsWithoutOperation.Add(item);
                        this.totalEnvelopesSeen++;
                        continue;
                    }
    ...
    internal sealed record RequestTelemetryEnvelope(
        ...
        TimeSpan Duration,
        IReadOnlyDictionary<string, string> Properties)
        : AzureMonitorTelemetryEnvelope("RequestData", Name, InstrumentationKey, Timestamp, OperationId, OperationParentId, Properties);
    ...
    internal sealed record ExceptionTelemetryEnvelope(
        ...
        string? TypeName,
        IReadOnlyDictionary<string, string> Properties)
        : AzureMonitorTelemetryEnvelope("ExceptionData", Name, InstrumentationKey, Timestamp, OperationId, OperationParentId, Properties);
    ...
    internal sealed record TraceTelemetryEnvelope(
        ...
        int? SeverityLevel,
        IReadOnlyDictionary<string, string> Properties)
        : AzureMonitorTelemetryEnvelope("MessageData", Name, InstrumentationKey, Timestamp, OperationId, OperationParentId, Properties);
    ...
    internal sealed record DependencyTelemetryEnvelope(
        ...
        bool Success,
        IReadOnlyDictionary<string, string> Properties)
        : AzureMonitorTelemetryEnvelope("RemoteDependencyData", Name, InstrumentationKey, Timestamp, OperationId, OperationParentId, Properties);
    ...
    internal static class AzureMonitorPayloadParser
    {
        public static IReadOnlyList<AzureMonitorTelemetryEnvelope> Parse(Request request)
        {
            ...
            if (request.Headers.TryGetValue("Content-Encoding", out var encoding) &&
                SplitAndTrimEntries(encoding).Any(value => value.Equals("gzip", StringComparison.OrdinalIgnoreCase)))
            {
        ...
        private static IReadOnlyList<AzureMonitorTelemetryEnvelope> ParseTelemetryPayload(Stream payloadStream)
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (TryParseTelemetry(line, out var envelope) && envelope != null)
                {
                    items.Add(envelope);
                }
            }
    ...
    private static bool TryParseTelemetry(string rawJson, out AzureMonitorTelemetryEnvelope? envelope)
        {
            ...
            var instrumentationKey = root.TryGetProperty("iKey", out var iKeyElement)
                ? iKeyElement.GetString() ?? string.Empty
                : string.Empty;

            var timestamp = root.TryGetProperty("time", out var timeElement) && DateTimeOffset.TryParse(timeElement.GetString(), out var parsedTime)
                ? parsedTime
            ...
            {
                case "RequestData":
                    envelope = ParseRequestTelemetry(baseData, name, instrumentationKey, timestamp, operationId, operationParentId, properties);
                    return envelope != null;

                case "ExceptionData":
                    envelope = ParseExceptionTelemetry(baseData, name, instrumentationKey, timestamp, operationId, operationParentId, properties);
                    return envelope != null;

                case "MessageData":
                    envelope = ParseTraceTelemetry(baseData, name, instrumentationKey, timestamp, operationId, operationParentId, properties);
                    return envelope != null;

                case "RemoteDependencyData":
                    envelope = ParseDependencyTelemetry(baseData, name, instrumentationKey, timestamp, operationId, operationParentId, properties);
                    return envelope != null;
            ...
            IReadOnlyDictionary<string, string> properties)
        {
            ...
            int? severity = null;
            if (baseData.TryGetProperty("severityLevel", out var severityElement) && severityElement.ValueKind == JsonValueKind.Number)
            {
...
#if NETFRAMEWORK
        private static IEnumerable<string> SplitAndTrimEntries(string value)
        {
    ...
    public override ValueTask ProcessAsync(HttpMessage message)
    {
        ...
        return CompleteValueTask();
    }

#if NETFRAMEWORK
    private static ValueTask CompleteValueTask() => default;
#else
    private static ValueTask CompleteValueTask() => ValueTask.CompletedTask;
#endif
    }
