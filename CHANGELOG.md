...
- [Fix #3211: `TelemetryClient.TrackDependency(DependencyTelemetry)` and `TelemetryClient.TrackRequest(RequestTelemetry)` now default an unset `Timestamp` to `DateTimeOffset.UtcNow` (restoring 2.x behavior) instead of emitting `0001-01-01T00:00:00Z`, which ingestion rejects.](https://github.com/microsoft/ApplicationInsights-dotnet/issues/3211)
