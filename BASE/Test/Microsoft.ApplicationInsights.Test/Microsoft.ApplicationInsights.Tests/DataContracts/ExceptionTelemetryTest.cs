namespace Microsoft.ApplicationInsights.DataContracts
{
    ...
    public class ExceptionTelemetryTest
    {
        ...
        public void ExceptionTelemetryDeepCloneCopiesAllProperties()
        {
            ...
            var other = (ExceptionTelemetry)telemetry.DeepClone();

            // Verify key properties are copied
            Assert.IsNotNull(other);
            Assert.AreEqual(telemetry.Message, other.Message);
            Assert.AreEqual(telemetry.SeverityLevel, other.SeverityLevel);
            Assert.AreEqual(telemetry.ProblemId, other.ProblemId);
            Assert.IsNotNull(other.Exception);
            Assert.AreEqual(telemetry.Exception.Message, other.Exception.Message);
            
            // Verify properties are copied
            foreach (var prop in telemetry.Properties)
            {
                Assert.IsTrue(other.Properties.ContainsKey(prop.Key), $"Property {prop.Key} should be copied");
                Assert.AreEqual(prop.Value, other.Properties[prop.Key], $"Property {prop.Key} value should match");
            }
        }

        [TestMethod]
        public void ExceptionTelemetryDeepCloneCopiesTimestampAndSequence()
        {
            var telemetry = new ExceptionTelemetry(new Exception("Test"));
            var timestamp = DateTimeOffset.UtcNow.AddMinutes(-10);
            telemetry.Timestamp = timestamp;
            telemetry.Sequence = "12345";
            
            var clone = (ExceptionTelemetry)telemetry.DeepClone();
            
            Assert.AreEqual(timestamp, clone.Timestamp);
            Assert.AreEqual("12345", clone.Sequence);
        }
