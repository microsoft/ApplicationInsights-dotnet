...
#pragma warning restore CA1724
    {
        ...
        public static void ValidateNumericAggregateValues(ITelemetry aggregate, string name, int count, double sum, double max, double min, double? stdDev)
        {
            ...
            else
            {
                ...
                {
                    Assert.AreEqual(stdDev.Value, metricAggregate.StandardDeviation.Value, Util.MaxAllowedPrecisionError, "metricAggregate.StandardDeviation mismatch");
                }
            }
        }

        public static void ValidateNumericAggregateValues(MetricAggregate aggregate, string name, int count, double sum, double max, double min, double stdDev, DateTimeOffset timestamp, long periodMs, string aggKindMoniker)
        {
        ...
        public static void ValidateNumericAggregateValues(MetricAggregate aggregate, string name, int count, double sum, double max, double min, double stdDev, string aggKindMoniker)
        {
            Assert.IsNotNull(aggregate);
            ...
            else if (aggregate.AggregationKindMoniker.Equals(MetricConfigurations.Common.Accumulator().Constants().AggregateKindMoniker))
            {
                Assert.AreEqual(sum, (double) aggregate.Data["Sum"], Util.MaxAllowedPrecisionError, "aggregate.Sum mismatch");
                Assert.AreEqual(max, (double) aggregate.Data["Max"], Util.MaxAllowedPrecisionError, "aggregate.Max mismatch");
                Assert.AreEqual(min, (double) aggregate.Data["Min"], Util.MaxAllowedPrecisionError, "aggregate.Min mismatch");
            }
            else if (aggregate.AggregationKindMoniker.Equals(MetricSeriesConfigurationForNaiveDistinctCount.Constants.AggregateKindMoniker))
            {
            ...
            else if (aggregate.AggregationKindMoniker.Equals(MetricConfigurations.Common.Gauge().Constants().AggregateKindMoniker))
            {
